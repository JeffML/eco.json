...
section('Check 1: No FEN collisions across eco files');
{
  ...
  for (const [file, entries] of Object.entries(fileEntries)) {
    for (const fen of Object.keys(entries)) {
      if (seen[fen]) {
        fail(`FEN appears in both ${seen[fen]} and ${file}: ${fen.slice(0, 60)}...`);
        collisions++;
      } else {
        seen[fen] = file;
      }
  ...
  if (collisions === 0) pass(`No FEN collisions across ${ECO_FILES.length} files`);
}
...
section('Check 2: All eco field values are valid (A00–E99)');
{
  ...
  if (bad === 0) pass(`All ${Object.keys(all).length} entries have valid ECO codes`);
}
...
section('Check 3: Exactly one isEcoRoot per ECO code (A01–E99)');
{
  ...
  if (trulyMissing.length === 0) {
    ...
    fail(`ECO codes present in data but missing isEcoRoot: ${trulyMissing.join(', ')}`);
  }
...
section('Check 5: All FENs referenced in fromTo.json exist in eco files');
{
  ...
  if (missing === 0) pass(`All ${fromTo.length} fromTo transitions reference valid FENs`);
}
