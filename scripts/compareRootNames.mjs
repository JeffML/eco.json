...
for (const [fen, entry] of Object.entries(before)) {
  if (entry.isEcoRoot) beforeRoots[entry.eco] = { fen, name: entry.name, src: entry.src };
}
for (const [fen, entry] of Object.entries(current)) {
  if (entry.isEcoRoot) currentRoots[entry.eco] = { fen, name: entry.name, src: entry.src };
}

const changed = [];
const unchanged = [];

const allCodes = new Set([...Object.keys(beforeRoots), ...Object.keys(currentRoots)]);
